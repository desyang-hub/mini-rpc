...
#define _RPC_STUB_METHOD(Class, Method) \
    ...
    using ReturnType_##Method = typename function_traits<MethodType_##Method>::return_type; \
    using ArgsTuple_##Method = typename function_traits<MethodType_##Method>::args_tuple; \
    \
    ...
    ReturnType_##Method Method(Args&&... args) { \
        ...
        if constexpr (std::is_void_v<ReturnType_##Method>) { \
            if (!RpcClient::call(srvName, args_tuple)) { \
                throw RpcException("RPC Call Failed: " + srvName); \
            } \
        } else { \
            if (!RpcClient::call(srvName, args_tuple, ret_val)) { \
                throw RpcException("RPC Call Failed: " + srvName); \
            } \
        } \
        \
